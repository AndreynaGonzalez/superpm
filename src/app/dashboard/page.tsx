...
const SIDEBAR = [
  { label: "Workspaces", icon: LayoutDashboard, active: true },
  { label: "Integraciones", icon: Plug, active: false },
  { label: "Design System", icon: Palette, active: false },
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-800/50">
              ...
              <p className="mt-4 text-xs text-zinc-400">
                Pegá este código en{" "}
                <span className="font-medium">mermaid.live</span> para
                renderizar el diagrama interactivo.
              </p>
            </div>
          </div>
        );
              ...
              {files.length > 0 && (
                <div className="mt-3 flex flex-wrap gap-2">
                  {files.map((f, i) => (
                    <span
                      key={`${f}-${i}`}
                      className="flex items-center gap-1 rounded-md bg-zinc-100 px-2 py-1 text-xs text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400"
                    >
                      {f}
                      <button
                        type="button"
          ...
          {!result && !loading && (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="mt-1 max-w-sm text-sm text-zinc-500 dark:text-zinc-400">
                Escribí la clave de un ticket de Jira y SuperPM generará el
                análisis completo con los 6 pilares de producto.
              </p>
            </div>
          )}
        </div>
